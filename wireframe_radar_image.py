...
def get_line_coords(path):
    with open(path, "rb") as f:  # bsps are binary files
        ...
        min_x = min(
            [p for i in [[vertex[0] for vertex in edge] for edge in edges] for p in i]
        )
        print(min_x)
        print(
            max(
                [
                    p
                    for i in [[vertex[0] for vertex in edge] for edge in edges]
                    for p in i
                ]
            )
        )
        min_y = min(
            [p for i in [[vertex[1] for vertex in edge] for edge in edges] for p in i]
        )
        min_z = min(
            [p for i in [[vertex[2] for vertex in edge] for edge in edges] for p in i]
        )
...
def create_line_image(edges, ax, x=0, y=1, title="", thickness=14):
    ...
    z = 3 - (x + y)
    print(
        max([p for i in [[vertex[0] for vertex in edge] for edge in edges] for p in i])
    )
    max_y = round(
        max([p for i in [[vertex[y] for vertex in edge] for edge in edges] for p in i])
    )
    img = Image.new(
        "RGB",
        (
            round(
                max(
                    [
                        p
                        for i in [[vertex[x] for vertex in edge] for edge in edges]
                        for p in i
                    ]
                )
            ),
            round(
                max(
                    [
                        p
                        for i in [[vertex[y] for vertex in edge] for edge in edges]
                        for p in i
                    ]
                )
            ),
    ...
    max_z = max(
        [p for i in [[vertex[z] for vertex in edge] for edge in edges] for p in i]
    )
    for edge in edges:
        mean_z = (edge[0][z] + edge[1][z]) / 2.0
        col = int(510 * mean_z / max_z)
        draw.line(
            (edge[0][x], max_y - edge[0][y], edge[1][x], max_y - edge[1][y]),
            fill=(max(0, col - 255), 0, max(0, 255 - col)),
            width=thickness,
        )
    if not ax:
        return img
